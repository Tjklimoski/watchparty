...
export async function DELETE(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (!watchParty.interestedUsersIds.includes(userId) && !user.interestedInWatchPartiesIds.includes(id)) {
      // user is alrady not following
      return res.json(watchParty)
    }

    // build new array for watchParty that doesn't contain the userId
    const interestedUsersIds = watchParty.interestedUsersIds.filter(interestedUserId => interestedUserId !== userId);

    // build new array for user that doesn't contain the watchParty id
    const interestedInWatchPartiesIds = user.interestedInWatchPartiesIds.filter(WPid => WPid !== id);
