...
export default function Achievments({ user }: AchievmentsProps) {
  ...
  const badges = [
    ...
    user.hosted_count >= 1 ? (
      <span key="hosted_1" className="badge badge-secondary">
        Hosted 1 WatchParty
      </span>
    ) : null,
    // Achievment for hosting 5 watchparties
    user.hosted_count >= 5 ? (
      <span key="hosted_5" className="badge badge-primary">
        Hosted 5 WatchParties
      </span>
    ) : null,
    // Achievment for hosting 10 watchparties
    user.hosted_count >= 10 ? (
      <span key="hosted_10" className="badge badge-accent">
    ...
    user.myWatchParties.length >= 1 ? (
      <span key="create_1" className="badge badge-accent">
    ...
    user.myList.length >= 1 ? (
      <span key="added_1" className="badge badge-secondary">
        Added 1 movie
      </span>
    ) : null,
    // Achievment for adding 10 movie to their list
    user.myList.length >= 10 ? (
      <span key="added_10" className="badge badge-primary">
        Added 10 movies
      </span>
    ) : null,
    // Achievment for adding 20 movie to their list
    user.myList.length >= 20 ? (
      <span key="added_20" className="badge badge-accent">
        Added 20 movies
      </span>
    ) : null,
    ...
    user.myWatchParties.some(WP => WP.partygoerIds.length - 1 >= 3) ? (
      <span key="hosted_3" className="badge badge-secondary">
        Hosted 3+ partygoers
      </span>
    ) : null,
